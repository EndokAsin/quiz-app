...
const loadProfilePage = async () => {
    ...
    document.getElementById('profile-form').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (file) {
                const bucketName = 'profile'; // Pastikan nama ini benar
...
const loadQuizForEditing = async (quizId) => {
    ...
    questionTypeSelect.addEventListener('change', () => {
        if (questionTypeSelect.value === 'essay') {
            mcOptionsContainer.classList.add('hidden');
            correctAnswerInput.placeholder = "Masukkan kunci jawaban/poin penting untuk esai";
        } else {
            mcOptionsContainer.classList.remove('hidden');
            correctAnswerInput.placeholder = "Contoh: A (untuk Pilihan Ganda)";
        }
    });
    if (timeLimitInput) {
        ...
        if (questions && questions.length > 0) {
            questions.forEach(q => {
                questionsListDiv.innerHTML += `<div class="p-4 bg-gray-100 rounded-md">
                    ...
                    ${q.question_image_url ? `<img src="${q.question_image_url}" class="max-w-xs rounded-md my-2">` : ''}
    ...
    document.getElementById('add-question-form').addEventListener('submit', async (e) => {
        ...
        const imageFile = document.getElementById('question-image-upload').files[0];
        ...
        if (imageFile) {
            const filePath = `${quizId}/${Date.now()}-${imageFile.name}`;
            const { error: uploadError } = await supabase.storage.from('questions').upload(filePath, imageFile);
        ...
        const { error } = await supabase.from('questions').insert({
            ...
            time_limit_seconds: parseInt(timeLimit, 10) || 0,
            options: options,
...
const loadQuizForStudent = async (quizId) => {
    ...
    const displayQuestion = () => {
        ...
        if (question.question_image_url) {
            imageContainer.innerHTML = `<img src="${question.question_image_url}" alt="Gambar Soal" class="max-w-md max-h-80 mx-auto rounded-lg mb-4">`;
        }
    ...
    const startQuestionTimer = (seconds) => {
        ...
        timerContainer.innerHTML = `<span id="timer">${seconds}</span>`;
